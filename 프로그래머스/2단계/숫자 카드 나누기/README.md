...
### 문제 풀이
1. 각 배열의 최대공약수를 먼저 구한다. ⇒ **유클리드 호제법 활용**
2. 각자의 최대공약수로 상대방의 배열값들이 나뉘는지 안나뉘는지 확인한다.
3. 두 배열 서로나뉘지 않는다면 a와 b 중 큰 최대공약수를 `return`한다.
4. 만약 하나라도 나뉜다면 `true`인 배열의 최대공약수를 `return`한다.
